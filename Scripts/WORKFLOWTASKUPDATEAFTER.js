...
| Action# : N/A
|
| Notes   : Updated on 2/18/2009 by Ean Darbo, added Function "CompleteCAP"
|
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
| <===========END=Main=Loop================>
/-----------------------------------------------------------------------------------------------------*/
//aa.sendMail("noreply@cityofmadison.com", "rsjachrani@cityofmadison.com", "rsjachrani@cityofmadison.com", "WTUA", debug);
...
function getGISCenterLine()
	{
	// This is for ROW permits to get GIS object information
...
function createRefLicProf(rlpId,rlpType,pContactType)
	...
	if (AInfo["Policy #"]) 			newLic.setPolicy(AInfo["Policy #"]);
        if (AInfo["Contractor License #"]) 	newLic.setContrLicNo(parseFloat(AInfo["Contractor License #"]));
...
function getLastInspector(insp2Check)
	...
	if (inspResultObj.getSuccess())
		...
		inspList.sort(compareInspDateDesc)
		for (xx in inspList)
			if (aa.person.getUser(inspList[xx].getInspector().getFirstName(),inspList[xx].getInspector().getMiddleName(),inspList[xx].getInspector().getLastName()).getOutput() != null)
			{
				inspUserObj = aa.person.getUser(inspList[xx].getInspector().getFirstName(),inspList[xx].getInspector().getMiddleName(),inspList[xx].getInspector().getLastName()).getOutput();
				return inspUserObj.getUserID();
			}
...
function compareInspDateDesc(a,b) 
	{
		try{if (a.getScheduledDate()!=null && b.getScheduledDate()!=null)
				{
...
function licenseObject(licnumber)
	...
	var licenseStatus = new Array("","Active","About To Expire","Delinquent","Expired","Invalid","Pending","Inactive");
...
function CompleteCAP(assignId) // option CapId
{
	var itemCap = capId
	if (arguments.length > 1) itemCap = arguments[1]; // use cap ID specified in args
	...
	cd.setCompleteDept(iName.getDeptOfUser());
	cd.setCompleteStaff(assignId);
	//cd.setCompleteDate(sysDate);  //Doesn't work 07/23/08 - Roland V.
	cdScriptObj.setCompleteDate(sysDate);
	...
	if (cdWrite.getSuccess())
	{ 	
		logDebug("Set CAP *Completed by Staff* to " + assignId) + "\nSet CAP *Completed by Dept* " + iName.getDeptOfUser() + "\nSet CAP *Completed Date* " + sysDate.toString(); 
	}
	else
	{ 	
		logDebug("**ERROR writing capdetail : " + cdWrite.getErrorMessage()) ;
