/*------------------------------------------------------------------------------------------------------/
| SVN $Id: PaymentReceiveAfter.js 3600 2008-10-27 21:36:24Z dane.quatacker $
| Program : PaymentReceiveAfterV1.5.js
...
function copyASIFields(sourceCapId,targetCapId)  // optional fields to ignore
	...
	for (ASICount in sourceASI)
		  ...
		  if (!exists(thisASI.getCheckboxType(),ignoreArray))
			   ...
			   if (!testresults.getSuccess()) //Let's try an update instead
			   	{
...
// Transfer Document from partial record to regular record.
TransferDocument();
...
function TransferDocument()
{
	...
	//move document from partial cap to regular cap
	var result = aa.cap.transferRenewCapDocument(partialCapID, capID);
	if(!result.getSuccess())
	{
		aa.print("ERROR: Failed to transfer document to regular id(" + capid + ")");
	}
}
