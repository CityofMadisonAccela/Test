...
| Action# : N/A
|
| Notes   : Added function loadAddressAttributes, dworonuk 12/17/2009
|
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
if(appTypeArray[0].substr(0,1) !="_") //Model Home Check --Riki
{
	var currentUserGroupObj = aa.userright.getUserRight(appTypeArray[0],currentUserID).getOutput()
	if (currentUserGroupObj) currentUserGroup = currentUserGroupObj.getGroupName();
}
...
var partialCap = !cap.isCompleteCap(); //Riki
...
logDebug("parentCapId = " + parentCapId);//Riki
logDebug("publicUser = " + publicUser); //RIKI
logDebug("partialCap = " + partialCap);//RIKI
...
function copyASIFields(sourceCapId,targetCapId)  // optional fields to ignore
	...
	for (ASICount in sourceASI)
		  ...
		  if (!exists(thisASI.getCheckboxType(),ignoreArray) && !exists(thisASI.getCheckboxDesc(),ignoreArray))
		       {
