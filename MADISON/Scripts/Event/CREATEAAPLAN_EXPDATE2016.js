/*------------------------------------------------------------------------------------------------------/
| Program:  CreateAAPlan_ExpDate2016
| Client: 
| Version 1.0 - CreateAAPlan_ExpDate2016 5/8/2014 Jane Schneider
|		This batch script is meant to be run one time time to create AA Plans for Prequal records
|		with Expiration record Expiration Date of 3/1/2016, as the original batch program was looking
|		only for records where the Expiration record Expiration Date had been 3/1/2015.
|
|				
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
/------------------------------------------------------------------------------------------------------*/
	...
	batchJobName = "CreateAAPlan_ExpDate2016";
...
/------------------------------------------------------------------------------------------------------*/
	...
	var capCount = 0;
	var newAAPlanCnt = 0;
	var AppTypeAAPlan = "Licenses/DCR/AA Plan/NA";	
...
/------------------------------------------------------------------------------------------------------*/
	...
	//Expiration date is March 1, 2016.
	var fromDate = "3/1/2016";
	var toDate = fromDate; 
	...
	{		
		...
		for (rec in myExp) {
			...
			if (!capId) {			
				...
				//if (appType.length && appMatch(appType) && AltID == "LPC2014057-01535") {   //For testing a single record.
				if (appType.length && appMatch(appType)){
					//logDebug("We have a Prequal record type match!");
					...
					//Create and update AA Plan child record, if one does not already exist.
						var holdId;
						var existingAAPlanCapId;
						var newAAPlanCapId;
						
						existingAAPlanCapId = childGetByCapType(AppTypeAAPlan, capId)
						//debugObject(existingAAPlanCapId);
						
						if (existingAAPlanCapId == null){
							logDebug("No existing child AA Plan, so let's create one!");
							newAAPlanCapId =  createChild("Licenses","DCR","AA Plan","NA", "Child of " + AltID, capId);
							
							logDebug("Variable type for newAAPlanCapId = " + typeof(newAAPlanCapId));
							logDebug("AA Plan CAP ID = " + newAAPlanCapId);
						
							if (typeof(newAAPlanCapId) == "object" && newAAPlanCapId != null){
								...
								newAAPlanCnt++
								holdId = capId; 
								capId = newAAPlanCapId;
								
								var contactArray = null; 
								var newCon = null;
								
								var oContacts = aa.people.getCapContactByCapID(newAAPlanCapId);
								...
								if(conAry != null){
									...
									for(con in conAry){								
										if (conAry[con].getPeople().contactType == "Business Contact") 
											aa.people.removeCapContact(newAAPlanCapId, conAry[con].getPeople().contactSeqNumber);
									}
								...
								switch(planStatus){
									case "APPROVED":
										...
										updateTask("AA Plan Status", "Active", updateComment, "");			//Set task status.
										setTask("AA Plan Status", "Y", "N", wfProcAAPlan);					//Make task active.
										updateAppStatus("Active", updateComment);
										break;
									case "EXEMPT":
										logDebug("In Switch at EXEMPT.");
										editAppSpecific("GENERAL INFO.Exempt", "CHECKED", newAAPlanCapId);
							...
							logDebug("An AA Plan already exists for Prequal record " + AltID + ".");
						}//End existing AA Plan check.													
					}//End CatB = true					
					logDebug(br);
		...
		logDebug("Total new AA Plan records created: " + newAAPlanCnt++);
	} //End mainProcess()
