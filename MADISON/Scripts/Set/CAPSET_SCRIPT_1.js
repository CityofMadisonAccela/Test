/*------------------------------------------------------------------------------------------------------/
| | Program : SetInvoiceFeesScript.js
...
for(var i=0; i < SetMemberArray.length; i++) {
  logMessage("SetMemberArray[" + i + "] is CAP ID:" + SetMemberArray[i])
  ...
  for (x in feeA) 
	...
	//aa.print(thisFee.code);
	}
  //aa.print("Its alt id =" + capId.getCustomID()); 
...
aa.env.setValue("ScriptReturnMessage", "Update Set successful - Invoice Script");
