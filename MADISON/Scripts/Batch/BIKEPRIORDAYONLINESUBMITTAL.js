...
//	toDate = new Date(dateAdd(toDate,0)); 
	...
	for (wf in capList) { 
		...
		if (capACA == true && jsDateToMMDDYYYY(capDate) == jsDateToMMDDYYYY(toDate) && 
			(capId.getID1().indexOf("TMP") < 0 && capId.getID1().indexOf("EST") < 0)){
	...
	aa.sendMail("noreply@cityofmadison.com", emailAddress, "elamsupport@cityofmadison.com", "Bike permit online application daily report", emailText);
	
	}
//To always send (if a negative confirmation is desired, uncomment below. ITGKL
