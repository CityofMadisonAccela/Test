...
| Client: Madison
| Version 1.0 Jeff Moyer 4/23/2013
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
|
...
var appTypeType = "Parks"; //app type to process
var appSubtype = "DiscGolf"; //app subtype to process
var appCategory = "NA"; //app category to process
var appType = appGroup + "/" + appTypeType + "/" + appSubtype + "/" + appCategory;
var expStatus = "Issued";
var resultStatus = "Closed";
var agency = "Madison";
...
function mainProcess() {
	var toDate = new Date("1/1/" + startDate.getFullYear());

	logDebug("This process all Disc Golf Permits that were issued in the previous calander year." + br);

	var wfResult = aa.workflow.getTasks("Permit Issuance", "Issued - Disc Golf");

	if (wfResult.getSuccess()) {
		workFlow = wfResult.getOutput();
		logDebug("The number of Disc Golf Permits in the - Issued - Disc Golf stage: " + workFlow.length + br);
	} else {
	...
	for (wf in workFlow) {
		...
		var issueDate = b1WF.getDueDate();
		
		debugObject(b1WF);
		
		//if (issueDate < todate) {
		//	logDebug(altId + " was closed" + br);
		//	closeTask(wfstr,wfstat,wfcomment,wfnote);
		//	closeTask(wfstr,wfstat,wfcomment,wfnote);
		//}
	}
	aa.sendMail("noreply@cityofmadison.com", emailAddress, "", "Vending Permit Close Report", emailText); // email Warren with results of closed caps
}
