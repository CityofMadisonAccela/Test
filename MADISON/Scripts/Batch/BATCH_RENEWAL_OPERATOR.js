...
|
/------------------------------------------------------------------------------------------------------*/
var fromDate =  "06/29/2014"; //;
var toDate =  "07/01/2014"; //;
var dFromDate = aa.date.parseDate(fromDate);			//
var dToDate = aa.date.parseDate(toDate);				//
var lookAheadDays = 1//aa.env.getValue("lookAheadDays");   // Number of days from today
var daySpan = 1//aa.env.getValue("daySpan");				// Days to search (6 if run weekly, 0 if daily, etc.)
...
var emailAddress = getParam("emailAddress");//"glabelle-brown@cityofmadison.com"			// email to send report
...
//var appTypeType = "*";				//   app type to process {Rental License}
	var appTypeType = "Clerk";
	//var appTypeType = "Fire";
	//var appTypeType = "Health";
var appSubtype = "Operator"	;  //			//   app subtype to process {NA}
var appCategory = "*"; //				//   app category to process {NA}

var expStatus =  "Active";//"About to Expire"			//   test for this expiration status
var newExpStatus = "About to Expire"; //	"Expired"	//   update to this expiration status
var newAppStatus = "About to Expire"; //	"Expired"	//   update the CAP to this status
var editWorkflowTask = "License Status";//"License Status"// Open this task {License Status},        
var newWorkflowStatus = "About to Expire";//"About to Expire""Expired"// // change to this status {Delinquent}
...
var emailAddress = "glabelle-brown@cityofmadison.com";//"glabelle-brown@cityofmadison.com"			// email to send report
var sendEmailNotifications = "N"//getParam("sendEmailNotifications");	// send out emails?
var sendSMSNotifications = "N"//getParam("sendSMSNotifications");	// send out SMS?
var removeSearchEntries = "N"//getParam("removeSearchEntries"); // remove search entries from DB
var mSubjChoice = ""//getParam("emailSubjectStdChoice");			// Message subject resource from "Batch_Job_Messages" Std Choice
var mMesgChoice = ""//getParam("emailContentStdChoice");			// Message content resource from "Batch_Job_Messages" Std Choice
var deactivateLicense = "N";//	"Y"		// deactivate the LP
var lockParentLicense = "N";// "N"    // add this lock on the parent license

var workflowFlowCode = ""; // Flow code for above status {U}
var assessRenewalFee= "N";// "N"              // Y or N fee codes come from b1Exp
var assessPenaltyFee= "N";//getParam("assessPenaltyFee");//               // Y or N
var invoiceFees= "N"; //"N"

//New Options for online renewals
var createPublicUser = "N";
var removeOldFees = "Y";
var onlineRenewal = "N";
...
function mainProcess()
	...
	for (thisExp in myExp)  // for each b1expiration (effectively, each license app)
		...
		if (appMatch("Licenses/Clerk/Operator/NA")) 
			onlineRenewal = "Y";
		else
			onlineRenewal = "N";
	...
	// Create Set RIKI EDIT
		...
		if (createPublicUser == "Y" && onlineRenewal == "Y") //we're only going to do this for caps that will have online renewals
		{
			createRefContactsFromCapContactsAndLink(capId,contactTypeArray,null,null,"N",comparePeopleGeneric); //RIKI RESEARCH
			//Find if there is a refContact already created
			//logDebug(altId);
		...
		if (removeOldFees == "Y") removeAssessedFees();
		
		if (assessPenaltyFee == "Y" && onlineRenewal == "N") // We're doing the penalty fee
			{
	...
	//logDebug("2:" + appTypeString);
		if (assessRenewalFee == "Y" && onlineRenewal == "N") // We're doing the renewalfee and not those that will occur online
			{
		...
		*/
		...
		//I'm adding a section here for app specific things that may need to hapen - Riki
		if (appMatch("Licenses/Engineering/Prequalified Contractor/Na"))
		{	
			if (newExpStatus.length > 0)
				editAppSpecific("License Status",newExpStatus);
			//there's a appspecfield that should replicate the b1exp status 
		}
		}
