...
batchJobName = "expireliquorclassbx2" + aa.env.getValue("BatchJobName");
wfObjArray = null;
var br = "<BR>";
...
var appSubtype = "ClassABeerLiquor"; //getParam("appSubtype");				//   app subtype to process {NA}
...
//normally would invoice, but first run after changes this year, assess first and check first 
...
function mainProcess() {
    ...
    aa.sendMail("noreply@cityofmadison.com", emailAddress, "", "Batch Job" + batchJobName, "At start of mainProcess function.");
    ...
    for (thisExp in myExp)  // for each b1expiration (effectively, each license app)
    {
        ...
        */
        ...
        if (assessPenaltyFee == "Y") // We're doing the penalty fee
        {
            ...
            /** Changes for 2013 - manually total to exclude 1st penalty fee*/
            feeAry = loadFees();
            ...
            for (fee in feeAry) {
                ...
                if ((feeAry[fee].code.search("PENALTY") == -1) && (feeAry[fee].amount - feeAry[fee].amountPaid) > 0 && feeAry[fee].status == "INVOICED") {
            ...
            penalty = subjToPenaltyTotal * .25;
            penalty = Math.round(100 * penalty) / 100;
            if (penalty > 0)
                addFee("PENALTYX2", aa.finance.getFeeScheduleByCapID(capId).getOutput(), "FINAL", penalty, "N");
