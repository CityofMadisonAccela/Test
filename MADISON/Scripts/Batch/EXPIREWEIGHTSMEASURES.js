/*------------------------------------------------------------------------------------------------------/
| Program: expireWeightsMeasures.js, based on Batch Expiration.js  Trigger: Batch
| Client: 
|
...
|
/------------------------------------------------------------------------------------------------------*/
var fromDate = "12/31/2014"; //getParam("fromDate"); 
var toDate = "01/01/2015"; //getParam("toDate");
...
var appGroup = "WeightsMeasures";  //getParam("appGroup");					//   app Group to process {Licenses}
var appTypeType = "Licensing";  //getParam("appTypeType");				//   app type to process {Rental License}
var appSubtype =  "NA";  //getParam("appSubtype");				//   app subtype to process {NA}
var appCategory = "NA";  //getParam("appCategory");				//   app category to process {NA}

var expStatus = "About to Expire";  //getParam("expirationStatus")			//   test for this expiration status
var newExpStatus = "Expired";  //getParam("newExpirationStatus")		//   update to this expiration status
...
var newWorkflowStatus = "Expired"; //"About to Expire"//getParam("newWorkflowStatus"); // change to this status {Delinquent}

var gracePeriodDays = 0;      		// getParam("gracePeriodDays")		//	bump up expiration date by this many days
var setPrefix = "WM2015PENALTY";	//getParam("setPrefix");			//   Prefix for set ID or some text to indicate want to add recs to set
var setName = "WM2015PENALTY";
var inspSched =  "";  //getParam("inspSched");					//   Schedule Inspection
var skipAppStatusArray = ""; //getParam("skipAppStatus").split(",")

var emailAddressTo = "elamsupport@cityofmadison.com"; //getParam("emailAddress");			// email to send report
var emailAddressCc = "jschneider@cityofmadison.com"; //getParam("emailAddress");			// email to send report
...
var workflowFlowCode = "U"; //getParam("workflowFlowCode");				 //Flow code for above status {U}
var assessRenewalFee = "N"; //getParam("assessRenewalFee")               // Y or N fee codes come from b1Exp
var assessPenaltyFee = "Y"; //getParam("assessPenaltyFee")               // Y or N
...
function mainProcess()
	...
	var capCount = 0;
	var capNoInvoiceCnt = 0;
	var inspDate;	
	var setDescription;	
	var exludeFromBilling = null;
	var qtyFarmMarketScale = null;
	
	aa.sendMail("noreply@cityofmadison.com", emailAddressTo, emailAddressCc, "Batch Job - " + batchJobName + "(" + sEnvironment + ")" , "At start of mainProcess function.");
	...
	for (thisExp in myExp)  // for each b1expiration (effectively, each license app)
		...
		//EXCLUDE BY CAP TYPE
		/*
		if (appMatch("Licenses/Clerk/ClassBBeerLiquor/NA") ||
			...
			appMatch("Licenses/Clerk/AdultEntertainment/NA") ||
                        appMatch("Licenses/Clerk/Scrap-Recycling/NA"))
			{
			capFilterType++;
			//logDebug(altId + ": Application Type does not match")
			continue;
	...
	// Create Set RIKI EDIT - not applicable, as we are hard-coding set name.
/*	
...
/* commented out by ITJBS, as not needed for W&M
...
*/
		...
		loadASITables();									// Riki - We have to add the ASI Table for the Fee Calculations
		//doStandardChoiceActions("PreExecuteForAfterEvents",true,0); //No Std Choice script called "PreExecuteForAfterEvents".
		//logGlobals(AInfo);
		
		//Check for records to exclude from assessing fees based on ASI field values.		
		var exludeFromBilling = null;
		var isFarmMarketScale = false;
		var deviceType = "";
		var deviceQty = 0;
		
		//First, check Exlude From Billing.
		if (AInfo["GENERAL INFORMATION.Exclude From Billing"] != null)
		{			
			logDebug("AInfo Exclude From Billing value: " + AInfo["GENERAL INFORMATION.Exclude From Billing"]);
			exludeFromBilling = AInfo["GENERAL INFORMATION.Exclude From Billing"].toString();
			//logDebug("Data Type of qtyFarmMarketScale: " + typeof(exludeFromBilling));
			logDebug("Value of Exclude from Billing = " + exludeFromBilling);
		}
		
		//Second, check if there is a FARM MARKET SCALE device.
		if (typeof(DEVICEINFORMATION) == "object")
		{
			for(x in DEVICEINFORMATION)
			{
				deviceType = DEVICEINFORMATION[x]["Device Type"].toString(); deviceQty = parseInt(DEVICEINFORMATION[x]["Number of Devices"].toString());				
				if(deviceType == "FARM MARKET SCALE")
				{
					//logDebug("data type for device type: " + typeof(deviceType));
					//logDebug("data type for device quant: " + typeof(deviceQty));
					isFarmMarketScale = true;
					break;
				}
			}
		}
		
        //No invoice if ASI field Exclude From Billing is checked or if device is a Farm Market Scale.
		if (exludeFromBilling == "CHECKED" || (isFarmMarketScale == true && deviceQty > 0))
		{
			capNoInvoiceCnt++
			logDebug(altId + ": record does not get invoice, as either Excluded From Billing or Farm Market Scale.");
			continue;
		}
	...
	//logDebug("2:" + appTypeString);
		...
		if (assessRenewalFee == "Y") // We're doing the renewalfee
			//logDebug("In assessRenewalFee.");
			{
				...
				else
				{
					...
					//logDebug("appFeeSched = " + appFeeSched);
		...
		*/
		...
		// Add to Set, unless we do not want to create an invoice report for the given record.
		if (setPrefix != "")
			{
				logDebug("In Set add section. Set Prefix = " + setPrefix + "; Set Name  = " + setName + "; Alt-ID = " + altId);
				aa.set.add(setName,capId); //RIKI RESEARCH
			}
			
		// lock Parent License - not applicable to W&M
				
		//if (lockParentLicense != "N") 
			//{
			//licCap = getLicenseCapId("*/*/*/*");   //This line throws an error if commented out within a comment block.
		/*
			if (licCap)
				{
	...
	logDebug("Number of CAPs neither assessed nor invoiced fees: " + capNoInvoiceCnt);
 	}
...
//Riki Needed to add so the fee script would run
...
function updateFee(fcode,fsched,fperiod,fqty,finvoice) // Updates a fee with a new Qty.  If it doesn't exist, adds it
	...
	//logDebug("fqty is of type " + typeof(fqty));
...
function editAppSpecific(itemName,itemValue)  // optional: itemCap
{
	...
	//if (itemName.substring(0,20) == "GENERAL INFORMATION."){
	//	itemName = itemName.substr(20)		
	//}
	...
	{		
		...
		if (itemName != "")
		{
			...
			{
				if(useAppSpecificGroupName == true) {
					sASILabel = appspecObj[i].getCheckboxType() + "." + appspecObj[i].getCheckboxDesc();
				}else{
					sASILabel = appspecObj[i].getCheckboxDesc();
				}
				
				//logDebug("ASILabel = " + sASILabel);
									
				if (sASILabel == itemName)
				{															
					...
					logDebug("function editAppSpecific: ASILabel " + sASILabel + " and item Name " + itemName + " are NOT equal");
				}
				i++;
			} // while loop
		} // item name blank
