/*------------------------------------------------------------------------------------------------------/
| Program: ExpireParkingPermitsDaily (modified from Batch Expiration.js) 
| Trigger: Batch
| Client: 
|
...
|
| ExpireParkingPermitsDaily 12/6/2012 Jane Schneider
|
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
...
|
/------------------------------------------------------------------------------------------------------*/
var lookAheadDays = -1; 		//Number of days from today; expire permits with expiration date of yesterday.
var daySpan = 0;										// Days to search (6 if run weekly, 0 if daily, etc.)

var fromDate = new Date(dateAdd(null,parseInt(lookAheadDays)));     //getParam("fromDate"); 
var toDate = fromDate; 												//getParam("toDate");

var appGroup = "Permitting";             		//getParam("appGroup");					// app Group to process {Licenses}
var appTypeType = "Parking";             		//getParam("appTypeType");				// app type to process {Rental License}
...
var newWorkflowStatus = "Expired";          	//getParam("newWorkflowStatus"); 		// change to this status {Delinquent}
...
var emailToAddress = "elamsupport@cityofmadison.com";       //getParam("emailToAddress");	// email to send report
var emailCcAddress = "jschneider@cityofmadison.com";  	    //getParam("emailCcAddress");	// email to send report
...
sExpireDateInfo = "Expiration Date = " + fromDate.toString() + "<br>";
var iMonth = fromDate.getMonth() + 1;
var iDayOfMonth = fromDate.getDate();
sExpireDateInfo = sExpireDateInfo + "Expiration Month = " + iMonth.toString() + "<br>";
sExpireDateInfo = sExpireDateInfo + "Expiration Day of month = " + iDayOfMonth.toString() + "<br>";
sMonthDay = iMonth.toString() + "/" + iDayOfMonth.toString();

fromDate = dateAdd(fromDate,0);
toDate = dateAdd(toDate,0);
sExpireDateInfo = sExpireDateInfo + "Date Range: From Date = " + fromDate + "; To Date = " + toDate;
logDebug(sExpireDateInfo);
...
if (paramsOK && sMonthDay != "8/31"){	
     logDebug("Expiration date is not 8/31, so start mainProcess.");

     if (!timeExpired) mainProcess();
     logDebug("End of Job: Elapsed Time : " + elapsed() + " Seconds");
}else{
     logDebug("Expiraton date is 8/31, so exiting batch job.<br>The annual Residential Parking Permit script, ExpireParkingPermits, needs to be run instead.");
}

if (emailToAddress.length)
	aa.sendMail("noreply@cityofmadison.com", emailToAddress, emailCcAddress, batchJobName + " batch job - Production Results", emailText);
...
function mainProcess()
	...
	//aa.sendMail("noreply@cityofmadison.com", emailToAddress, emailCcAddress, "Production Batch Job - " + batchJobName, "At start of mainProcess function.");
