...
|
/------------------------------------------------------------------------------------------------------*/
var fromDate =  "01/31/2013"; //"06/29/2011";
var toDate =  "01/31/2013"; //"07/01/2011";
...
var appGroup = "Licenses"; //"Licenses"					//   app Group to process {Licenses}
var appTypeType = "Engineering"; //"Building Inspection"				//   app type to process {Rental License}
var appSubtype = "Prequalified Contractor";  //	"*"			//   app subtype to process {NA}
...
var newWorkflowStatus = "Expired"// // change to this status {Delinquent}
...
var deactivateLicense = "Y"		// deactivate the LP
var lockParentLicense = "N"    // add this lock on the parent license

var workflowFlowCode = getParam("workflowFlowCode"); // Flow code for above status {U}
var assessRenewalFee= "N"              // Y or N fee codes come from b1Exp
var assessPenaltyFee= "N"              // Y or N
var invoiceFees= "N"
/*----------------------------------------------------------------------------------------------------/
|
...
function mainProcess()
	...
	// Create Set RIKI EDIT
		...
		// Actions start here:
                ...
                var refLicNo = altId;
                if(refLicNo.substring(0,3).toUpperCase().equals("LPC") ){
                //Do ROW & TER here, deactivate general prequal below
                    var rsx = parseInt(altId.substring(altId.length() - 5),10)+"";
                    refLicNo = "ROW"+rsx;      
                    refLic = getRefLicenseProf(refLicNo);
			...
			logDebug(refLicNo + ": deactivated linked License");
	            }
                    refLicNo = "TER"+rsx;      
                    refLic = getRefLicenseProf(refLicNo);
                    if (refLic && deactivateLicense.substring(0,1).toUpperCase().equals("Y"))
