var maxSeconds = 60 * 30;							// number of seconds allowed for batch processing, usually < 5*60
...
function mainProcess()
	{
		...
		rsCutOffDate = new Date("07/01/" + rsYear);
		firstSchedule = "07/01/" + rsYear;
		...
		if (FDCAP.getSuccess())
		{
			...
			for(var iRecords=0; iRecords < FDCAPs.length; iRecords++) 
			{
				...
				if (lastInspection == null) 
				{
					...
					countSecond++;
					updateAppStatus("Second Routine");
					continue;
				}
				...
				countSecond++;
				if (capStatus == "Active" || capStatus == "Annual Routine" || capStatus == "First Routine") 
				{
					updateAppStatus("Second Routine");
				}
