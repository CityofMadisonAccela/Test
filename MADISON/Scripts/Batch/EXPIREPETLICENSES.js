...
| Version 1.4 - Updated to run against Pet Licenses 7/29/2013 ITJSM
|
/------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
|
...
function mainProcess() {
	var activeStartDate = new Date("10/01/" + startDate.getFullYear());//new Date("11/01/" + startDate.getFullYear());
	var activeEndDate = new Date("10/31/" + startDate.getFullYear());//new Date("11/30/" + startDate.getFullYear());
	var aboutStartDate = new Date("01/01/" + startDate.getFullYear());
	var aboutEndDate = new Date("01/31/" + startDate.getFullYear());
	...
	if (startDate >= activeStartDate && startDate <= activeEndDate) {
		...
		processExpiration();
		fromDate = "12/31/" + (startDate.getFullYear() - 2);
		toDate = "01/01/" + (startDate.getFullYear() - 1);
		curStatus = "Expired";
		newStatus = "Inactive";
