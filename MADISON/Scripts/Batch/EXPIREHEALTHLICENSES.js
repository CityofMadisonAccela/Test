...
var maxSeconds = 60 * 60;							//Prod; number of seconds allowed for batch processing, usually < 5*60
...
|
/------------------------------------------------------------------------------------------------------*/
var fromDate = "06/30/2013"; //getParam("fromDate"); 
var toDate = "06/30/2013"; //getParam("toDate");
...
var appTypeType = "Health"; //getParam("appTypeType");				//   app type to process {Rental License}
var appSubtype =  "*";  //getParam("appSubtype");				//   app subtype to process {NA}
var appCategory = "*";  //getParam("appCategory");				//   app category to process {NA}

//Below - should be About to Expire in Production
...
var sendEmailNotifications = "Y"; //getParam("sendEmailNotifications");	// send out emails?
...
var invoiceFees= "Y"; //getParam("invoiceFees");		        //Prod
/*----------------------------------------------------------------------------------------------------/
|
...
if (emailAddress.length)
	aa.sendMail("noreply@cityofmadison.com", emailAddress, "elamsupport@cityofmadison.com", batchJobName + " Results", emailText);
...
function mainProcess()
	...
	var setDescription;
        ...
        aa.sendMail("noreply@cityofmadison.com", emailAddress, "elamsupport@cityofmadison.com", "Batch Job - expireHealthLicenses", "At start of mainProcess function.");
		...
		if (assessPenaltyFee == "Y") // We're doing the penalty fee
			{
				...
				if (appMatch("Licenses/Health/School/DaneCounty") ||
				appMatch("Licenses/Health/School/Madison"))
				{
					capFilterType++;
					logDebug(altId + ": Application Type does not match")
					continue;
				...
				/**Change notes 2013************************************************************************************
				*  For health fees, only PHMDC fees are applicable to the fee. We've tried to use an SG fee in the past
				*  unfortunately that also includes already paid fees with the same SG in it's calculation, 
				*  which requires manual updating of older records SG code each year. 
				*  Non stormwater discharge and Beach don't have PHMDC included in the renewal fee code.
				*  School CAPs, no penalty
				********************************************************************************************************/

				//debugObject(aa.finance.getFeeScheduleByCapID(capId));
				removeAssessedFees();
				feeAry= loadFees();
				var subjToPenaltyTotal = 0;
				var penalty = 0;
				for(fee in feeAry){
					//logDebug("Fee code is: "+feeAry[fee].code + " Amount is: "+ feeAry[fee].amount + " Amount Paid: " + feeAry[fee].amountPaid);
					if((feeAry[fee].code.search("PHMDC") != -1 || feeAry[fee].code == "DISCH"|| feeAry[fee].description == "Beach License Renewal Fee") && (feeAry[fee].amount - feeAry[fee].amountPaid) > 0 && feeAry[fee].status == "INVOICED") {
						subjToPenaltyTotal = subjToPenaltyTotal + (feeAry[fee].amount - feeAry[fee].amountPaid);
					}
				}
				//logDebug("subjToPenaltyTotal:"+subjToPenaltyTotal);
				penalty = subjToPenaltyTotal * .15;
				penalty = Math.round(100 * penalty)/100;
				//Uncomment below for comparison purposes with old penalty, if running this script with InvoiceFee parameter set to N
				//var feObj = updateFee(b1Exp.getPenaltyCode(),aa.finance.getFeeScheduleByCapID(capId).getOutput(),b1Exp.getPayPeriodGroup(),1,"N");
				if(penalty > 0)           
					addFee("NEW_PENALTY", "LICFD201", "FINAL", penalty, "N");
				
			/*    appFeeSched = lookup("BATCH:FeeScheduleLookupComplexPenalty",appTypeString)
			    if (appFeeSched != null)
			    {
